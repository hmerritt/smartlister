...
//  check if user is on a mobile or tablet
...
function waves(obj, opacity) {
    ...
    $(obj).prepend(
        '<div id="' +
            xObj +
            '" class="waves" style="background-color: ' +
            xopacity +
            ';"></div>'
    );
    setTimeout(function() {
        $(xObjId)
            .addClass("active")
            .css({
                top: "calc(50% - " + xObjValHalf + "px)",
                left: "calc(50% - " + xObjValHalf + "px)",
                width: xObjVal + "px",
                height: xObjVal + "px",
                "border-radius": xObjVal + "px"
            });
    }, 50);
    setTimeout(function() {
        $(xObjId).remove();
...
$(document).ready(function() {
    ...
    if (
        Cookies.get("directoryLister") == null ||
        Cookies.get("directoryLister") == undefined
    ) {
    ...
    function updateCookie(cookieName, setting, value) {
        try {
            ...
            Cookies.set(cookieName, Base64.encode(JSON.stringify(cookie)), {
                expires: 9999
            });
        } catch (error) {
    ...
    $(window).on("beforeunload", function(e) {
        var unUploadedFiles = $(".files-container .directory .uploading")
            .length;
    ...
    $(document).keyup(function(e) {
        ...
        if (e.which == 13 || e.keyCode == 13) {
            var activeItem = $(
                ".files-container .directory[id=" +
                    md5(directory) +
                    "] .item.active:not(.uploading, .new, .deleting)"
            );
        ...
        if (e.which == 38 || e.keyCode == 38) {
            ...
            var items = $(
                    ".files-container .directory[id=" +
                        md5(directory) +
                        "] .item:not(.uploading, .new, .deleting)"
                ),
                firstFolder = $(
                    ".files-container .directory[id=" +
                        md5(directory) +
                        "] .folders .item:not(.uploading, .new, .deleting)"
                ).first(),
                lastFolder = $(
                    ".files-container .directory[id=" +
                        md5(directory) +
                ...
                firstFile = $(
                    ".files-container .directory[id=" +
                        md5(directory) +
                        "] .files .item:not(.uploading, .new, .deleting)"
                ).first(),
                lastFile = $(
                    ".files-container .directory[id=" +
                        md5(directory) +
    ...
    function getFolder(folder) {
        if (
            typeof content[md5(folder)] === "undefined" ||
            content[md5(folder)] === null
        ) {
            ...
            $.ajax({
                ...
                .done(function(data) {
                    try {
                        ...
                        if (data["status"] == "ok") {
                            ...
                            content[folderHash]["files"] = content[folderHash][
                                "files"
                            ].sort(objSort("name", sort));
                            content[folderHash]["folders"] = content[
                                folderHash
                            ]["folders"].sort(objSort("name", sort));

                            console.log(
                                "[Files] Cached items: " +
                                    content[folderHash]["folders"].length +
                                    " folders, " +
                                    content[folderHash]["files"].length +
                                    " files (" +
                                    folder +
                                    ")"
                            );
                        ...
                        console.error(
                            "[Files] Unable to request items from folder: " +
                                folder +
                                " (" +
                                err +
                                ")"
                ...
                .fail(function() {
                    ...
                    console.error(
                        "[Files] Unable to request items from folder; will try again in 1 second (most likely due to loss of internet)"
                    );
                });
        } else {
            ...
            try {
                ...
                toast("Unable to get files", "cross", 4000);
                console.error(
                    "[Files] Unable to retreive cached items from folder: " +
                        folder +
                        " (" +
                        err +
                        ")"
    ...
    function addDirectory(dir) {
        ...
        var directoryHTML = $("<div/>")
                .addClass("directory")
                .attr("id", dirHash)
                .append('<div class="folders"></div><div class="files"></div>'),
        ...
        if (items["files"].length > 0) {
            for (var item in items["files"]) {
                ...
                $(fileHTML).append(
                    "\n                  <div class='item flex no-user-select' uploadId='" +
                        uploadId +
                        "' link='" +
                        Base64.encode(link) +
                        "' type='" +
        ...
        if (items["folders"].length > 0) {
            for (var item in items["folders"]) {
                var folder = items["folders"][item],
                    ...
                    link = folder["link"];
                $(folderHTML).append(
                    "\n                  <div class='item flex no-user-select' name='" +
                        name +
                        "' link='" +
                        Base64.encode(link) +
        ...
        //  reset rename btn
        $(".info-items .item.rename, .info-items .item.delete").addClass(
            "faded"
        );
        ...
        window.history.pushState(
            newTitle,
            newTitle,
            "index.php?directory=" + urlDirectory
        );
    }
    ...
    function getFileIcon(type, link, size) {
        if (!link.includes("data:image")) {
            link =
                settings["listerFolderName"] +
                ("/php/image.php?size=" + size + "&fileDirectory=" + link);
        }
    ...
    $(document).mouseup(function(e) {
        ...
        if (
            !$(e.target).closest(
                ".files-container .item:not(.uploading, .new), .contextmenu, .info-items .item.rename, .info-items .item.delete, button.modal__btn.modal__btn-primary.modal-delete__btn-primary"
            ).length > 0
        ) {
    ...
    $(document).on(
        ...
        ".files-container .item:not(.uploading, .new)",
        function(e) {
            if ($(this).hasClass("active")) {
                return false;
            }
            $(".files-container .item").removeClass("active");
            $(".info-items .item.rename, .info-items .item.delete").removeClass(
                "faded"
            );
    ...
    $(document).on(
        action,
        ".files-container .directory .folders .item:not(.new, .deleting, .renaming)",
        function(e) {
    ...
    $(document).on(
        action,
        ".files-container .directory .files .item:not(.uploading, .renaming), .info-items .item[link]",
        function(e) {
            try {
                var dlink =
    ...
    $(document).mouseup(function(e) {
        //  do not run if clicked specific objects
        if (
            !$(e.target).closest(".info-items .item:not(.storage, .delete)")
                .length > 0
        ) {
        ...
        if (
            $(e.target).closest(".info-items .newFolder").length == 0 &&
            $(e.target).closest(".folders .item.new").length == 0
        ) {
            removeNewFolder();
        }
    ...
    $(document).on(
        "click",
        ".info-items .item:not(.storage, .newFolder)",
        function() {
            if ($(this).hasClass("active")) {
                return false;
            }
    ...
    function getStorageQuota() {
        $.ajax({
            ...
            try {
                ...
                $(".info-items .item.storage h1 small").html(
                    content["storage"]
                );

                console.log(
                    "[Storage] Retrieved folder size: " + content["storage"]
                );
            } catch (err) {
    ...
    function triggerUpload() {
        ...
        if (files.length > 0 && settings["fileUpload"]) {
            //  show main loader
            $(".cloud-save-state .spinner")
                .removeClass("hidden")
                .addClass("fadeIn");
            $(".cloud-save-state .cloud-saved")
                .addClass("hidden")
                .removeClass("fadeIn");
    ...
    function upload(fileNumber, fileId) {
        ...
        content[md5(directory)]["files"] = content[md5(directory)][
            "files"
        ].sort(objSort("name", sort));
        var fileObj =
            "\n          <div class='item uploading flex no-user-select' uploadId='" +
            fileId +
            "' link='" +
            Base64.encode(link) +
            "' type='" +
        ...
        if (fileCount > 1) {
            ...
            $(
                ".files-container .directory[id=" + md5(directory) + "] .files"
            ).prepend(fileObj);
            thisObj = $(
                ".files-container .directory[id=" +
                    md5(directory) +
            ...
            if (window.FileReader) {
                if (Base64.decode(type).includes("image")) {
                    ...
                    reader.onloadend = function(e) {
                        $(
                            ".files-container .files .item[uploadId=" +
                                fileId +
                                "]"
                        )
                            .find(".img")
        ...
        $.ajax({
            ...
            .done(function(data) {
                ...
                $(".files-container .files .item[uploadId=" + fileId + "]")
            ...
            .fail(function(data) {
                ...
                for (var file in content[md5(fileInfo["directory"])]["files"]) {
                    var link =
                        content[md5(fileInfo["directory"])]["files"][file][
                            "link"
                        ];
                    if (link == fileInfo["link"]) {
                        content[md5(fileInfo["directory"])]["files"].splice(
                            file,
                            1
                        );
                        $(
    ...
    $(document).on(
        "contextmenu",
        ".files-container .item:not(.uploading, .new, .newFolder)",
        function(e) {
            var left = e.clientX,
                top = e.clientY;
    ...
    $(document).on("mouseup", ".contextmenu .item", function(e) {
        ...
        var activeFile = $(
                ".files-container .directory[id=" + directory + "] .item.active"
            ),
            type = "file",
    ...
    function addNewFolder() {
        ...
        $(".files-container .directory[id=" + md5(directory) + "] .folders")
            .prepend(newFolderHTML)
            .find("input")
            .focus()
            .parent()
            .parent()
    ...
    function removeNewFolder() {
        try {
            $(".files-container .directory .folders .item.new")
    ...
    $(document).on(
        "keyup",
        ".files-container .directory .folders .item.new input",
        function(e) {
            e.preventDefault();
    ...
    function newFolder(name) {
        ...
        for (var folder in content[md5(directory)]["folders"]) {
            if (
                content[md5(directory)]["folders"][folder][
                    "name"
        ...
        $.ajax({
            ...
            .done(function(data) {
                try {
                    ...
                    console.error(
                        "[Folder] Unable to create a new folder (" + err + ")"
                    );
                }
            })
            .fail(function() {
                toast("Folder not created", "cross", 4000);
                console.error(
                    "[Folder] Unable to request a new folder (most likely due to loss of internet)"
                );
            });
    }
    ...
    function deleteItem() {
        ...
        $.ajax({
                ...
                try {
                    ...
                    if (data["status"] == "ok") {
                        ...
                        for (var item in content[md5(dir)][plType]) {
                            if (
                                content[md5(dir)][plType][item]["name"] == name
                            ) {
            ...
            .fail(function() {
                ...
                toast("Item not deleted", "cross", 4000);
                console.error(
                    "[Item] Unable to request for an item deletion (most likely due to loss of internet)"
                );
            });
    }
    ...
    function renameToggle(toggle) {
        ...
        if (
            toggle == true &&
            item.length == 1 &&
            !$(item).hasClass("renaming") &&
            !$(item).hasClass("new") &&
            !$(item).hasClass("uploading")
        ) {
            //  save old name
            //  set input html
            renameOldName = $(item)
                ...
                .html();
            var input =
                '<div class="input"><input placeholder="Rename item" type="text"></div>';
    ...
    function renameItem(type, oldName, newName) {
        ...
        if (type == "folder") {
            for (var folder in content[md5(directory)]["folders"]) {
                if (
                    content[md5(directory)]["folders"][folder][
                        "name"
                    ].toLowerCase() == newName.toLowerCase()
                ) {
                    toast("Folder already exists", "cross", 4000);
                    throw new Error("[Folder] Duplicate found");
                }
            ...
            for (var file in content[md5(directory)]["files"]) {
                if (
                    content[md5(directory)]["files"][file][
                        "name"
                    ].toLowerCase() == newName.toLowerCase()
                ) {
        ...
        $.ajax({
            ...
            .done(function(data) {
                try {
                    ...
                    if (data["status"] == "ok") {
                        ...
                        for (var crum in breadcrums) {
                            linkFile +=
                                encodeURIComponent(breadcrums[crum]) + "/";
                        }
                        ...
                        if (type == "folder") {
                            ...
                            for (var item in content[md5(dir)]["files"]) {
                                if (
                                    content[md5(dir)]["files"][item]["name"] ==
                                    oldName
                                ) {
                                    content[md5(dir)]["files"][item][
                                        "name"
                                    ] = newName;
                                    content[md5(dir)]["files"][item][
                                        "link"
            ...
            .fail(function() {
                toast("Item not renamed", "cross", 4000);
                console.error(
                    "[Item] Unable to request for a rename (most likely due to loss of internet)"
                );
            });
    }
    ...
    if (settings["moveItems"]) {
        //  detect file drag
        $(function() {
            var pressed, posX, posY, objPressed;

            //  detect mouse down + move + up / item hover
            $(document)
                .on(
                    "mousedown",
                    ".directory .item:not(.new, .uploading, .renaming)",
                    function(e) {
                        pressed = true;
                        posX = e.clientX;
                        posY = e.clientY;
    ...
    function itemDragStart(item, posX, posY) {
        ...
        //  add icon
        if (icon["type"] == "svg") {
            $(".drag-to-move .name").before(
                '<svg viewBox="0 0 24 24">' + icon["html"] + "</svg>"
            );
        } else {
            $(".drag-to-move .name").before(
                '<div class="img" style=\'' + icon["html"] + "'></div>"
            );
        }
    }
    ...
    function itemDragEnd() {
        ...
        $(".directory .item, .bredcrums .item").removeClass(
    ...
    function moveItem(item) {
        ...
        var name = $(item)
            ...
            type = "file",
            dir = directory,
            newDirectoryName = $(".directory .folders .item.drag-active")
                .find(".name")
                .html(),
            newDirectoryBreadcrums = $(".breadcrums .item.drag-active").attr(
                "dir"
            );
        ...
        if (newDirectoryName !== undefined && newDirectoryName !== null) {
            ...
            newDirectoryBreadcrums !== undefined &&
            newDirectoryBreadcrums !== null
        ) {
            //  decode and parse breadcrum directory
            var newBreadcrums = JSON.parse(
                Base64.decode(newDirectoryBreadcrums)
            );
        ...
        if (
            JSON.stringify(currentBreadcrums) ==
                JSON.stringify(newBreadcrums) ||
            newBreadcrums[newBreadcrums.length - 1] == name
        )
            return false;
        ...
        $.ajax({
                    ...
                    if (data["status"] == "ok") {
                        ...
                        for (var item in content[md5(dir)][plType]) {
                            if (
                                ...
                                //  update item link
                                itemObj["link"] =
                                    newDirectory.substring(4) +
                                    "/" +
                                    itemObj["name"];

                                //  add item to new directory (if it exists)
                                if (
                                    typeof content[md5(newDirectory)] !==
                                        "undefined" &&
                                    typeof content[md5(newDirectory)] !== "null"
                                ) {
                                    content[md5(newDirectory)][plType].push(
                                        itemObj
                                    );
                                }
                            }
            ...
            .fail(function() {
                toast("Item not moved", "cross", 4000);
                console.error(
                    "[Item] Unable to request for an item move (most likely due to loss of internet)"
                );
            });
    }
